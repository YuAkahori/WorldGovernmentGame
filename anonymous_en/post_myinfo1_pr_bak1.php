<html>
<head><title>SIZOOKA AREA</title></head>
<body>
...
print('シゾーカ県内での居場所を表示します。137-x-139.5 34-y-36 <br>');

pg_set_client_encoding("utf8");

$result = pg_query('SELECT * FROM wgg_location_sc.wgg_location_a1_tb  ');
...
for ($i = 0 ; $i < pg_num_rows($result) ; $i++){
    ...
    print('specialid='.$rows['special_id']);
...
<canvas id=canvas001 width=500 height=400></canvas>
<script>
...
window.onload = function() {
 ...
 context001.strokeRect(0,0,500,400);
...
//<?php $n3= 1?>
//var lx1=<?php echo $location_x[$n3]; ?> ;
//var ly1=<?php echo $location_y[$n3]; ?> ;
...
function move(val1=138,val2=35){
...
// 横幅が500 縦幅が400 ある
// xは、137から139.5の間。差し引き2.5　250 500 2.5＊200 200bai
// yは　34から36の間 差し引き2 　200 400 ２*200　400 200倍している
//升目が１０あるから、５を引く
...
if (137<=val1 &&  val1<=139.5)
     {l_now_x = (val1-137)*200-5};

let l_now_y =  -50;
if (34<=val2 && val2<=36)
     {l_now_y = 　400- (val2-34)*200  -5};
...
button {
 ...
 <form action="post_myinfo1_pr.php" method="post">
...
//up
    ...
    if (isset($_POST['bt_up'])) {
        header("Location:post_up01_a1_01.php");
    }
//dowm
    elseif (isset($_POST['bt_down'])) {
        header("Location:post_down01_a1_01.php");
        }
//left
    elseif (isset($_POST['bt_left'])){
        header("Location:post_left01_a1_01.php");
        }
//right
elseif (isset($_POST['bt_right'])){
        header("Location:post_right01_a1_01.php");
        }
//４　リスト表示
